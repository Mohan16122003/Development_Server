...
const SalesOrderInfo = () => {
  ...
  const fetchSalesOrder = () => {
    toast
      .promise(httpService.get(`/sale-order/${id}`), {
  ...
  const { isAccountManager, isAdmin } = useSelector(
    (val) => val.authentication
  );
  const handleDelete = () => {
    toast
      .promise(httpService.delete(`/sale-order/${id}`), {
        ...
        <div className="d-flex">
          ...
          <div className="p-2 ml-2 flex-fill">
            <div>
              ...
              <div className="ml-3">
                <Link
                  to={`/app/profile/customer-profile/${sOData?.customer?._id}`}
                >
                ...
                <div>
                  {sOData?.customer?.billingAddress?.city} -{' '}
                  {sOData?.customer?.billingAddress?.state}
                </div>
  ...
  return (
    <div className="page-wrapper">
      ...
      <div className="content container-fluid">
                ...
                <>
                  {
                    <>
                      ...
                      {(!sOData?.isInvoiced && isAdmin) ||
                        (isAccountManager && (
                          <>
                            <Link
                              to={{
                                pathname: '/app/sales/createsalesorder',
                                state: { ...sOData, edit: true },
                              }}
                              className="rounded-circle bg-primary p-2 float-right mr-2 text-light"
                            >
                              <Edit />
        ...
        {viewPDF ? pdfView() : SalesOrderView()}
