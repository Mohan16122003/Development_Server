...
const Payments = () => {
  ...
  useEffect(async () => {
    if (state?.customerInv && state?.customerId) {
      ...
      const res = await httpService.get(
        `/sale-payment/customerinvpay?customerId=${state?.customerId}&customerInv=${state?.customerInv}`
      );
      setData(res.data);
  ...
  const handleDelete = async () => {
    ...
    setDataToRender(
      data.filter((payment) => payment._id !== paymentToEdit._id)
    );
  ...
  function searchTable(data) {
    let newData = data
      .filter(
        (row) =>
          row.paymentNumber?.toLowerCase()?.indexOf(qpay.toLowerCase()) > -1
      )
      .filter(
        (c) =>
