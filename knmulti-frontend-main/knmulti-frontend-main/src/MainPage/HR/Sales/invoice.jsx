...
const Invoice = () => {
  ...
  useEffect(async () => {
    if (state?.customerId) {
      const res = await httpService.get(
        `/customer/invoices/${state?.customerId}`
      );
      setData(res.data);
  ...
  function searchTable(data) {
    let newData = data
      .filter(
        (row) => row.invoice?.toLowerCase()?.indexOf(qinv.toLowerCase()) > -1
      )
      .filter(
        (c) =>
    ...
    if (qdateby == 'duedateby') {
      ...
      if (qdateto !== '') {
        newData = newData.filter(
          (fd) => fd?.invoiceDate.split('T')[0] <= qdateto
        );
      }
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <span>
          ₹ {record.grandTotal - record.paidAmount - record?.withholdingTax}
        </span>
      ),
      sorter: (a, b) => a.amount.length - b.amount.length,
    },
      ...
      <div className="content container-fluid">
        ...
        <div className="page-header">
          <div className="row align-items-center">
            ...
            {(isAdmin || isAccountManager || isSalesManager) && (
