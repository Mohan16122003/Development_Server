...
const Estimates = () => {
  ...
  useEffect(async () => {
    if (state?.customerId) {
      const res = await httpService.get(
        `/customer/estimate/${state?.customerId}`
      );
      setData(res.data);
  ...
  const filterDate = (e) => {
    ...
    setDataToRender((dataToRender) =>
      dataToRender.filter(
        (data) => new Date(data.estimateDate?.split('T')[0]) > date
      )
    );
  };
  ...
  const filterCustomerName = (e) => {
    ...
    setDataToRender((dataToRender) =>
      dataToRender.filter((data, i) =>
        data.customer?.displayName?.includes(e.target.value)
      )
    );
  };
  ...
  const filterEstimateNumber = (e) => {
    ...
    setDataToRender((dataToRender) =>
      dataToRender.filter((data, i) => data.estimate?.includes(e.target.value))
    );
  };
  ...
  const handleDelete = async () => {
    ...
    setDataToRender(
      data.filter((estimate) => estimate._id !== estimateToEdit._id)
    );
  ...
  function searchTable(data) {
    let newData = data
      .filter(
        (row) => row.estimate?.toLowerCase()?.indexOf(qest.toLowerCase()) > -1
      )
      .filter(
        (c) =>
          c.customer?.displayName
            .toLowerCase()
            .indexOf(qcustomer.toLowerCase()) > -1
      );
    ...
    if (qdateby == 'expdateby') {
      ...
      if (qdateto !== '') {
        newData = newData.filter(
          (fd) => fd?.expiryDate.split('T')[0] <= qdateto
        );
      }
    } else {
      ...
      if (qdateto !== '') {
        newData = newData.filter(
          (fd) => fd?.estimateDate.split('T')[0] <= qdateto
        );
      }
    }
  ...
  const columns = [
    {
      ...
      dataIndex: 'estimate',
      render: (text, record) => (
        <Link to={{ pathname: `/app/sales/estimatesview/${record._id}` }}>
          {text}
        </Link>
      ),
...

    {
      ...
      render: (text, record) => <p>{record?.estimateDate?.split('T')[0]}</p>,
      sorter: (a, b) => a.estimateDate.length - b.estimateDate.length,
    },
    {
      title: 'Expiry Date',
      dataIndex: 'expiryDate',
      render: (text, record) => <p>{record?.expiryDate?.split('T')[0]}</p>,
    ...
    },
    {
      ...
      render: (text, record) => (
        <Badge
          status={text?.toUpperCase() === 'INVOICED' ? 'success' : 'warning'}
