...
const RecordPayment = () => {
  ...
  console.log('invoice data =>',constantInvoice)
  const handleClick = () => {
    setAmount(constantInvoice[0].balance);
    let withhTax = 0;
    let payAmount = 0;
    let index = 0;
    setRecordPayment({
      ...recordPayment,
      amountReceived: parseInt(constantInvoice[0].balance)
    });
    if (index >= 0) {
      const newInvoiceList = [...invoices];
      const updatedInvoice = {
        ...invoices[index],
        paidAmount: parseInt(constantInvoice[0].balance) + constantInvoice[index].paidAmount,
      };
      newInvoiceList[index] = updatedInvoice;
      setInvoices(newInvoiceList);
      newInvoiceList.map((newInvoice, i) => {
                          ...
                          <tr className="text-center" key={index}>
                            ...
                            <td>
                              <input
                                ...
                                type="number"
                                value={amount}
                                max={recordPayment.amountReceived||0}
                                min={0}
                                onWheel={e => e.currentTarget.blur()}
                                onChange={(e) => {
