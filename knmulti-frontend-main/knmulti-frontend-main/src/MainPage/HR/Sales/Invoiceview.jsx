...
const InvoiceView = () => {
  ...
  const fetchInvoices = async () => {
    toast
      .promise(httpService.get(`/sale-invoice/${id}`), {
  ...
  const fetchCredits = async (customerId) => {
    toast
      .promise(httpService.get(`/sale-payment?customer=${customerId}`), {
  ...
  const handleDelete = () => {
    toast
      .promise(httpService.delete(`/sale-invoice/${id}`), {
  ...
  const handleConfirmStatus = () => {
    toast
      .promise(httpService.put(`/sale-order/${id}`, { status: 'CONFIRMED' }), {
          ...
          <div className="p-2 ml-2 flex-fill">
            {invData?.customer?.billingAddress && (
              <div>
                ...
                <div className="ml-3">
                  <Link
                    to={`/app/profile/customer-profile/${invData?.customer?._id}`}
                  >
                  ...
                  <div>
                    {invData?.customer?.billingAddress?.city} -{' '}
                    {invData?.customer?.billingAddress?.state}
                  </div>
                  <div>
                    INDIA - {invData?.customer?.billingAddress?.zipcode}
                  </div>
                </div>
              </div>
            )}
                ...
                <div className="row">
                  <div className="col-6">
                    <Link
                      className="btn btn-primary continue-btn"
                      to={{
                        pathname: `/app/sales/payment-received`,
                        state: {
                          customerId: invData?.customer?._id,
                          customerInv: invData?._id,
                        },
                      }}
                      onClick={(e) => {
                        e.preventDefault();
  ...
  return (
    <div className="page-wrapper">
      ...
      <div className="content container-fluid">
        ...
        <div className="page-header">
          <div className="row">
            <div className="col d-flex">
              <h3 className="page-title">INVOICE :- {invData?.invoice} </h3>{' '}
              <span
                className={
                  invData?.status == 'PAID'
                ...
                <>
                  <Link
                    ...
                    to={{
                      ...
                      state: { ...invData, edit: true, sOConvert: true },
                    }}
                  >
                    Record Payment
                  </Link>
            ...
            <div className="row mt-2">
              <div className="col">
                ...
                >
                  ...
                  <strong>₹ {customerPayCreditAmount}</strong>{' '}
                  <Link
                    className="text-gray ml-2"
                    to={{
                      pathname: `/app/sales/apply-credits`,
                      state: {
                        customerPayCreditInfo,
                        invID: invData?._id,
                        invNo: invData?.invoice,
                        invBalDue: invData?.balance,
                        invData,
                      },
                    }}
                  >
        ...
        {viewPDF ? pdfView() : InvoiceInfoView()}

        {isDelete ? DeleteInvPopUp() : GoToPayPopUp()}
      </div>
    </div>
  );
};
