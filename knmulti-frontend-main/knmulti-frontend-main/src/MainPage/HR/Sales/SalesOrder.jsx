...
const SalesOrder = () => {
  ...
  useEffect(async () => {
    if (state?.customerId) {
      const res = await httpService.get(
        `/customer/orders/${state?.customerId}`
      );
      setData(res.data);
  ...
  const filterSalesOrder = (saleOrder) => {
    if (saleOrder === '') {
      setDataToRender([...data]);
      return;
    }
    setDataToRender((dataToRender) =>
      dataToRender.filter((data, i) => data?.salesOrder?.includes(saleOrder))
    );
  };
    ...
    if (qdateby == 'expdateby') {
      ...
      if (qdateto !== '') {
        newData = newData.filter(
          (fd) => fd?.shipmentDate.split('T')[0] <= qdateto
        );
      }
    } else {
      ...
      if (qdateto !== '') {
        newData = newData.filter(
          (fd) => fd?.orderDate.split('T')[0] <= qdateto
        );
      }
    }
  ...
  useEffect(() => {
    filterCustomerName(qcustomer);
  }, [qcustomer]);

  useEffect(() => {
    filterSalesOrder(qso);
  }, [qso]);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Badge
          status={text?.toUpperCase() === 'CONFIRMED' ? 'success' : 'warning'}
