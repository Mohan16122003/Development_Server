...
const RecurringInvoiceInfo = () => {
  ...
  const fetchRecurringInvInfo = () => {
    toast
      .promise(httpService.get(`/recurring-invoice/${id}`), {
  ...
  const deleteRecurringInvoice = () => {
    toast
      .promise(httpService.delete(`/recurring-invoice/${id}`), {
      ...
      <div>
        <div className="d-flex">
          <div className="p-2 mr-2 flex-fill">
            <span className="badge bg-warning p-2 h5">
              {recurringinvoiceData?.status}
            </span>
            <table className="table table-borderless">
              <tbody>
                <tr>
                  <td>Repeats</td>
                  <td>
                    {recurringinvoiceData?.frequency} times a{' '}
                    {recurringinvoiceData?.frequencyUnit}
                  </td>
                </tr>
                <tr>
                  <td>Starts On </td>
                ...
                <tr>
                  <td>Ends On</td>
                  <td>
                    {recurringinvoiceData?.neverExpires === true
                      ? 'Goes on forever'
                      : recurringinvoiceData?.endDate?.split('T')[0]}
                  </td>
                </tr>
                <tr>
                  <td>PAYMENT TERMS</td>
          ...
          <div className="p-2 ml-2 flex-fill">
            ...
            <Link
              to={`/app/profile/customer-profile/${recurringinvoiceData?.customer?._id}`}
            >
              <p>{recurringinvoiceData?.customer?.displayName}</p>
            </Link>
            <div>
              {recurringinvoiceData?.customer?.billingAddress?.attention}
            </div>
            <div>
              {recurringinvoiceData?.customer?.billingAddress?.addressLine1}
            </div>
            <div>
              {recurringinvoiceData?.customer?.billingAddress?.addressLine2}
            </div>
            <div>
              {recurringinvoiceData?.customer?.billingAddress?.city} -{' '}
              {recurringinvoiceData?.customer?.billingAddress?.state} -{' '}
              {recurringinvoiceData?.customer?.billingAddress?.zipcode}
            </div>
        ...
        <div>
          <table className="table table-striped">
            ...
            <tbody>
              {recurringinvoiceData?.items?.length > 0 &&
                recurringinvoiceData?.items.map((trx, index) => (
                  <tr key={index}>
                    <td>
          ...
          <div className="flex-grow-1">
            ...
            <div className="d-flex justify-content-between">
              ...
              <p className="h4 text-muted">
                (-)₹{recurringinvoiceData?.discount}
              </p>
            </div>
            <div className="d-flex justify-content-between">
              <p className="h4 text-muted">Tax</p>
              <p className="h4 text-muted">
                ₹{recurringinvoiceData?.withholdingTax}
              </p>
            </div>
            {recurringinvoiceData?.adjustment?.adjustmentValue !== 0 && (
              <div className="d-flex justify-content-between">
                ...
                <p className="h4 text-muted">
                  ₹{recurringinvoiceData?.adjustments}
                </p>
              </div>
            )}
            <hr />
        ...
        <div>
          ...
          <p className="text-muted">
            Customer Notes: {recurringinvoiceData?.customerNotes}
          </p>
          <p className="text-muted">
            Terms And Conditions: {recurringinvoiceData?.termsAndConditions}
          </p>
        </div>
        {/* } */}
      </div>
          ...
          <div className="row">
            <div className="col">
              <h3 className="page-title">
                Recurring Invoice :- {recurringinvoiceData?.profileName}
              </h3>
            </div>
            <div className="col">
              {(isAdmin || isSalesManager) && (
                <Link
                  to={{
                    pathname: '/app/sales/recurring-invoices-create',
                    state: { recurringinvoiceData, edit: true },
                  }}
              ...
              <Link
                ...
                to={{
                  ...
                  state: { recurringinvoiceData, convertRecurring: true },
                }}
              >
                Create invoice
              </Link>
