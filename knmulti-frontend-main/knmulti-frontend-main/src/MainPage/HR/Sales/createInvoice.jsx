  ...
  const handleSubmit = async (e) => {
    ...
    if (state?.estConvert) {
      ...
      const response = await toast
        .promise(httpService.post(`/sale-invoice/`, invoice), {
          pending: 'Creating the Invoice',
          success: 'Invoice created successfully',
        })
        .catch((err) => {
          toast.error(
            `Error in creating Invoice ! ${err.response?.data?.errors[0]?.message}`
          );
        });
      ...
      const plot = await projectToModify?.subPlots.find(
        (p) => p?.name == plotName
      );
      ...
      if (plotName) {
        response.data.project.subPlots
          ...
          .leadsInfo.find(
            (l) => l?.customer == response?.data?.customer?._id
          ).leadType = 'Booking';
