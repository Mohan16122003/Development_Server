...
const PurchaseOrderView = () => {
  ...
  useEffect(() => {
    toast
      .promise(httpService.get(`/vendortrx/getvendorpurchaseorder${search}`), {
  ...
  function searchTable(data) {
    let newData = data
      .filter(
        (row) =>
          row.purchaseOrderNo.toLowerCase().indexOf(qpo.toLowerCase()) > -1
      )
      .filter(
        (r) =>
    ...
    if (qdateby == 'edlydateby') {
      ...
      if (qdateto !== '') {
        newData = newData.filter(
          (fd) => fd?.expentedDeliveryDate.split('T')[0] <= qdateto
        );
      }
    } else {
      ...
      if (qdateto !== '') {
        newData = newData.filter(
          (fd) => fd?.purchareOrderDate.split('T')[0] <= qdateto
        );
      }
    }
  ...
  const columns = [
    {
      ...
      align: 'center',
      render: (purchareOrderDate) => (
        <span>{dateFormatter(purchareOrderDate?.split('T')[0])}</span>
      ),
    },
    {
      ...
      render: (text, record) => (
        <span>
          <Link to={`/app/purchase/purchaseorderinfo/${record._id}`}>
            {text}
          </Link>
        </span>
      ),
    ...
    },
    {
      ...
      align: 'center',
      render: (expentedDeliveryDate) => (
        <span>{dateFormatter(expentedDeliveryDate?.split('T')[0])}</span>
      ),
    },
    {
      title: 'BILLED STATUS',
      ...
      <div className="content container-fluid">
            ...
            <div className="col-auto float-right ml-auto">
              <Link
                to={{
                  ...
                  state: { purchaseOrderLength: purchaseOrders?.length },
                }}
        ...
        <div className="row filter-row justify-content-between">
          ...
          <div
            ...
            style={{ width: '12.499999995%', maxWidth: '12.3%' }}
          >
            <div className="form-group form-focus focused">
              <input
                className="form-control floating"
          ...
          <div className="col-sm-6 col-md-2 col-lg-3 col-xl-1 col-12">
            <button
