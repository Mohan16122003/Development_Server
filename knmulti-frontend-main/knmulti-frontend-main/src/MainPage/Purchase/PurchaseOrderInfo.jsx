...
const PurchaseOrderInfo = () => {
  ...
  const fetchVendorBillInfo = () => {
    toast
      .promise(httpService.get(`/vendortrx/getvendorpurchaseorder?_id=${id}`), {
  ...
  const deletepurchaseOrder = () => {
    toast
      .promise(httpService.delete(`/vendortrx/removepurchaseorder/${id}`), {
  ...
  const markCancel = () => {
    toast
      .promise(
        httpService.put(`/vendortrx/updatepurchaseorder/${id}`, {
          status: 'CANCELLED',
        }),
        {
      ...
      <>
        <div className="d-flex">
          <div className="p-2 mr-2 flex-fill">
            ...
            <table className="table table-borderless">
              <tbody>
                <tr>
                  <td style={{ fontSize: '0.8rem' }}>REFERENCE#</td>
                ...
                <tr>
                  <td style={{ fontSize: '0.8rem' }}>DELIVERY DATE</td>
                  <td>{pOData?.expentedDeliveryDate?.split('T')[0]}</td>
                </tr>
                <tr>
                  <td style={{ fontSize: '0.8rem' }}>PAYMENT TERMS</td>
                  <td>{pOData?.paymentTerms}</td>
                </tr>
                <tr>
                  <td style={{ fontSize: '0.8rem' }}>PROJECT NAME</td>
                  <td>
                    <Link
                      to={`/app/projects/projects-view/${pOData?.projectId?._id}`}
                    >
          ...
          <div className="p-2 ml-2 flex-fill">
            <div>
              ...
              <div className="ml-3">
                <Link
                  to={`/app/profile/vendor-profile/${pOData?.vendorId?._id}`}
                >
                  <div>{pOData?.vendorId?.name}</div>
                </Link>
                <div>{pOData?.vendorId?.billAddress}</div>
              </div>
            </div>
            <hr />
            <div>
              ...
              {pOData?.deliveryTo == 'Customer' && (
                <>
                  ...
                  <div className="ml-3">
                    <Link
                      to={`/app/profile/customer-profile/${pOData?.customerId?._id}`}
                    >
                    ...
                    <div>
                      {pOData?.customerId?.billingAddress?.addressLine1},{' '}
                      {pOData?.customerId?.billingAddress?.addressLine2}
                    </div>
                    <div>
                      {pOData?.customerId?.billingAddress?.city} -{' '}
                      {pOData?.customerId?.billingAddress?.state}
                    </div>
                    <div>
                      {pOData?.customerId?.billingAddress?.country} -{' '}
                      {pOData?.customerId?.billingAddress?.zipcode}
                    </div>
                  </div>
                </>
              )}
              {pOData?.deliveryTo == 'Organisation' && (
                <>
                  ...
                  <div className="ml-3">
                    ...
                    <pre style={{ fontFamily: 'Lato', fontSize: '1rem' }}>
                      {pOData?.organisationData?.address}
                    </pre>
                  </div>
                </>
              )}
        ...
        <div>
          <table className="table table-striped">
            ...
            <tbody>
              {pOData?.transaction?.length > 0 &&
                pOData?.transaction.map((trx) => (
                  <tr>
            ...
            {pOData?.adjustment?.adjustmentValue !== 0 && (
              <div className="d-flex justify-content-between">
                <p className="h4 text-muted">
                  {pOData?.adjustment?.adjustmentName}
                </p>
                <p className="h4 text-muted">
                  ₹{pOData?.adjustment?.adjustmentValue}
                </p>
              </div>
            )}
            <hr />
  ...
  return (
    <div className="page-wrapper">
      ...
      <div className="content container-fluid">
        ...
        <div className="page-header">
          <div className="row">
            <div className="col d-flex">
              <h3 className="page-title mr-2">
                Purchase Order# {pOData?.purchaseOrderNo} Customer
              </h3>
              <div>
                <span
                  className={
                    pOData?.status === 'ISSUED'
                      ? 'badge bg-success text-secondary p-1 h6 ml-2'
                      : pOData?.status == 'CANCELLED'
                      ? 'badge bg-danger p-2 h5 ml-2'
                  ...
                  {pOData?.billedStatus == 'BILLED' ? (
                    <div className="btn btn-primary float-right mr-2">
                      <Link
                        className="text-light"
                        to={{
                          pathname: `/app/purchase/billinfo/${pOData?.billInfo}`,
                        }}
                      >
                        View Bill
                      </Link>
                      ...
                      <div className="btn btn-primary float-right mr-2">
                        <Link
                          ...
                          to={{
                            ...
                            state: { pOData: pOData, pOConversion: true },
                          }}
                        >
                          Convert to Bill
                        </Link>
        ...
        {viewPDF ? pdfView() : PurchaseOrderView()}

        <DeleteModel title="Purchase Order" fn={deletepurchaseOrder} />
        <FileUploadModel
          modLink={`/vendortrx/updatepurchaseorder/${pOData?._id}`}
          filesInfo={pOData?.fileInfos}
        />
        <>
          <div
