...
const ExpensesView = () => {
  ...
  useEffect(() => {
    toast
      .promise(httpService.get(`/vendortrx/getvendorexpense${search}`), {
        error: 'Failed to fetch Expenses',
        success: 'Expenses Details fetched successfully',
        pending: 'fetching Expenses...',
      })
      .then((res) => {
        setExpenseData(res.data.reverse());
  ...
  function searchTable(data) {
    ...
    if (qdateto) {
      newData = newData.filter(
        (fd) => fd?.expenseDate.split('T')[0] <= qdateto
      );
    }
  ...
  const columns = [
    {
      ...
      align: 'center',
      render: (expenseDate) => (
        <span>{dateFormatter(expenseDate?.split('T')[0])}</span>
      ),
    },
    {
      ...
      render: (text, record) => (
        <span>
          <Link to={`/app/purchase/expenseinfo/${record._id}`}>{text}</Link>
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (customerId) => (
        <span key={customerId?._id}>
          {' '}
          <Link to={`/app/profile/customer-profile/${customerId?._id}`}>
    ...
    <div className="page-wrapper">
      ...
      <div className="content container-fluid">
        ...
        <div className="page-header">
          <div className="row">
            <div className="col">
              <h3 className="page-title">All Expense</h3>
            </div>
