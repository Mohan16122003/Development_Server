...
function Vendor() {
  ...
  const fetchVendors = async () => {
    ...
    console.log('response from vendor=>', res);
    setData(
  ...
  function searchTable(data) {
    let newData = data
      .filter(
        (row) => row.name?.toLowerCase().indexOf(qName.toLowerCase()) > -1
      )
      .filter((r) => r.email?.toLowerCase().indexOf(qEmail.toLowerCase()) > -1)
      .filter(
        (c) => c.company?.toLowerCase().indexOf(qCompany.toLowerCase()) > -1
      );

    if (qtype) {
      newData = newData.filter(
        (s) => s.vendorType?.toLowerCase()?.indexOf(qtype?.toLowerCase()) > -1
      );
    }
    return newData;
  }
  ...
  const columns = [
    {
      ...
      render: (text, record) => (
        <h2 className="table-avatar">
          <Link
            to={`${
              record?.vendorType !== 'landowner'
                ? `/app/profile/vendor-profile/${record._id}`
                : `/app/land-bank/profile/${record._id}`
            }`}
            className="avatar"
          >
            <div
            ...
            >
              {record.fileInfos[0]?.filePath ? (
                <img
                  src={record.fileInfos[0]?.filePath || '/profile'}
                  alt=""
                  style={{
                    width: '100%',
                    height: '100%',
        ...
        <h2 className="table-avatar">
          {text == 'landowner' ? (
            <span className="">
              ...
              <img
                src="https://byw10.agumentikgroup.com/images/house-owner.png"
                width="40"
                height="30"
                alt="aaa"
              />
          ...
          {text == 'agent' ? (
            <span className="">
              <img
                src="https://byw10.agumentikgroup.com/images/agent.png"
                width="40"
                height="30"
                alt="aaa"
              />
          ...
          {text == 'supplier' ? (
            <span className="">
              {/* <i className="fa fa-truck" /> */}
              <img
                src="https://byw10.agumentikgroup.com/images/hotel-supplier.png"
                width="40"
                height="30"
                alt="aaa"
              />
      ...
      render: (text, record) => (
        <Space size={'small'}>
          <Link
            className="ant-btn"
            to={{
              pathname: `/app/profile/vendor-profile/${record._id}`,
            ...
            <div className="row align-items-center">
              ...
              {(isAdmin || isSalesManager) && (
                <div className="col-auto float-right ml-auto">
                  <Link to="/app/purchase/addvendor" className="btn add-btn">
              ...
              {(isAdmin || isSalesManager) && (
                <div className="col-auto float-right ml-auto">
                  <Link to="/app/land-bank/add-land" className="btn add-btn">
                    <i className="fa fa-plus" /> Add Land Owner
                  </Link>
                </div>
              )}
            </div>
