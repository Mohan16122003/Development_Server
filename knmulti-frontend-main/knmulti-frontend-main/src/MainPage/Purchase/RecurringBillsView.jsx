...
const RecurringBillsView = () => {
  ...
  useEffect(() => {
    toast
      .promise(httpService.get(`/vendortrx/getrecurringbill`), {
  ...
  function searchTable(data) {
    ...
    if (qdateto) {
      newData = newData.filter(
        (fd) => fd?.billNextDate.split('T')[0] <= qdateto
      );
    }
  ...
  const columns = [
    {
      ...
      render: (text, record) => (
        <span>
          <Link to={`/app/purchase/recurringbillInfo/${record._id}`}>
            {text}
          </Link>
        </span>
      ),
    ...
    },
    {
      ...
      align: 'center',
      render: (billNextDate) => (
        <span>{dateFormatter(billNextDate?.split('T')[0])}</span>
      ),
    },
    {
      title: 'Status',
