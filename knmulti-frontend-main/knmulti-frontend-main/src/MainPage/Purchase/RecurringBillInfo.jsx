...
const RecurringBillInfo = () => {
  ...
  const fetchVendorRecurringBillInfo = () => {
    toast
      .promise(httpService.get(`/vendortrx/getrecurringbill?_id=${id}`), {
  ...
  const deleteVendorRecurringBill = () => {
    toast
      .promise(httpService.delete(`/vendortrx/removerecurringbill/${id}`), {
  ...
  const stopBillRecur = () => {
    toast
      .promise(
        httpService.put(
          `/vendortrx/updaterecurringbill/${recurringBillData?._id}`,
          { status: 'INACTIVE' }
        ),
        {
      ...
      <div>
        <div className="d-flex flex-row">
          <div className="p-2 mr-2 flex-fill">
            ...
            {recurringBillData?.status === 'ACTIVE' ? (
              <span className="badge bg-success text-white p-2 h5">ACTIVE</span>
            ) : (
              <span className="badge bg-danger text-white p-2 h5">
                INACTIVE
              </span>
            )}
              ...
              <tbody>
                <tr>
                  ...
                  <td style={{ fontSize: '1rem' }}>
                    In {recurringBillData?.repeatEvery?.repeatNumber}{' '}
                    {recurringBillData?.repeatEvery?.repeatUnit} intervals
                  </td>
                </tr>
                <tr>
                  <td>Starts On </td>
                ...
                <tr>
                  <td>Ends On</td>
                  <td>
                    {recurringBillData?.neverExpire === true
                      ? 'Goes on forever'
                      : recurringBillData?.billEndDate?.split('T')[0]}
                  </td>
                </tr>
                <tr>
                  <td>Payment Terms</td>
          ...
          <div className="p-2 ml-2 flex-fill mt-4">
            <p className="h5">Vendor</p>
            <Link
              to={`/app/profile/vendor-profile/${recurringBillData?.vendorId?._id}`}
            >
              <p>{recurringBillData?.vendorId?.name}</p>
            </Link>
            <div>{recurringBillData?.vendorId?.billAddress}</div>
          </div>
        </div>
        ...
        <div>
          <table className="table table-striped">
            ...
            <tbody>
              {recurringBillData?.transaction?.length > 0 &&
                recurringBillData?.transaction.map((trx, index) => (
                  <tr key={index}>
        ...
        <div className="d-flex justify-content-between">
          ...
          <div className="flex-grow-1">
            ...
            <div className="d-flex justify-content-between">
              <p className="h4 text-muted" style={{ fontSize: '1rem' }}>
                Discount
              </p>
              <p className="h4 text-muted" style={{ fontSize: '1rem' }}>
                (-)₹{recurringBillData?.discountAmount}
              </p>
            </div>
            <div className="d-flex justify-content-between">
              <p className="h4 text-muted" style={{ fontSize: '1rem' }}>
              ...
              <p className="h4 text-muted" style={{ fontSize: '1rem' }}>
                ₹{recurringBillData?.taxAmount}
              </p>
            </div>
            {recurringBillData?.adjustment?.adjustmentValue !== 0 && (
              <div className="d-flex justify-content-between">
                <p className="h4 text-muted" style={{ fontSize: '1rem' }}>
                  {recurringBillData?.adjustment?.adjustmentName}
                </p>
                <p className="h4 text-muted" style={{ fontSize: '1rem' }}>
                  ₹{recurringBillData?.adjustment?.adjustmentValue}
                </p>
              </div>
            )}
            <hr />
          ...
          <div className="row">
            <div className="col">
              <h3 className="page-title">
                Recurring Bill :- {recurringBillData?.profileName}
              </h3>
            </div>
            <div className="col">
              {!isSalesEmployee && (
                <div className="rounded-circle bg-primary p-2 float-right mr-2">
                  <Link
                    to={{
                      ...
                      state: { recurringBillData, edit: true },
                    }}
              ...
              <div className="btn btn-primary float-right mr-2">
                <Link
                  ...
                  to={{
                    ...
                    state: { recurringBillData, convertRecurring: true },
                  }}
                >
                  Create Bill
                </Link>
