...
const AddRecurringBill = () => {
  ...
  let handleTaxAmount = () => {
    if (taxType !== '') {
      if (taxSystem === 'TCS') {
        console.log(adjustment.adjustmentValue);
        let updatedAmount =
          subTotal - discountAmount + Number(adjustment.adjustmentValue);
  ...
  const handleGrandTotal = () => {
    let grandTotal =
      ...
      discountAmount -
      taxAmount +
      Number(adjustment.adjustmentValue);
    setTotal(grandTotal);
  };
  ...
  const sortedItems = [...vendors].sort((a, b) => {
    // Assuming each item has a "created_at" property in ISO 8601 format
  ...
  useEffect(() => {
    if (state?.edit) {
      ...
      setName(vendorId?._id||ownerId._id);
      setProjectId(projectId?._id);
      setProfileName(profileName);
      setRepeatEvery(repeatEvery);
      setbillStartDate(billStartDate?.split('T')[0]);
      setbillEndDate(billEndDate?.split('T')[0]);
  ...
  const handleSubmit = (e) => {
    ...
    // console.log({ recurringBillFinalData });
    if (state?.edit) {
      toast
        .promise(
          httpService.put(
            `/vendortrx/updaterecurringbill/${state?.recurringBillData?._id}`,
            {
              ...recurringBillFinalData,
              status: state?.recurringBillData?.status,
            }
          ),
          {
        ...
        .then((res) => {
          dispatch(
            createNotify({
              notifyHead: `Recurring Bill ${
                res?.data?.profileName || recurringBillFinalData?.profileName
              }`,
              notifyBody: `Recurring Bill ${
                res?.data?.profileName || recurringBillFinalData?.profileName
              } got updated`,
              createdBy: empId,
            })
      ...
      toast
        .promise(
          httpService.post(
            `/vendortrx/createrecurringbill`,
            recurringBillFinalData
          ),
          {
            error: 'Failed to create recurring Bill',
            success: 'recurring Bill created successfully',
            pending: 'Building vendor recurring Bill...',
          }
        )
        .then((res) => {
          dispatch(
  ...
  return (
    <div className="page-wrapper">
      <Helmet>
        ...
        <meta name="description" content="Recurring bill" />
      </Helmet>
      {/* Page Content */}
        ...
        <div className="page-header">
          <div className="row">
            <div className="col-sm-12">
              <h3 className="page-title">Recurring Bill</h3>
              <ul className="breadcrumb">
                <li className="breadcrumb-item">
                  <Link to="/app/dashboard">Dashboard</Link>
                </li>
                <li className="breadcrumb-item active">Recurring Bill</li>
              </ul>
            </div>
          </div>
        </div>
