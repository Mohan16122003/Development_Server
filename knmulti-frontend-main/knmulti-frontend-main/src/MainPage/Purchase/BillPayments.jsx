...
const BillPayments = () => {
  ...
  const [paymentNo, setPaymentNo] = useState(
    `PYM-${Math.ceil(Math.random() * 100000)}`
  );
  ...
  const [referenceId, setReferenceId] = useState(
    `RTX-${Math.ceil(Math.random() * 100000)}`
  );
  ...
  const fetchVendorBill = async () => {
    if (vendor) {
      const { data } = await httpService.get(
        `/vendortrx/getvendorbills?vendorId=${vendor}`
      );
      ...
      let updatedVendorBillData = vendorBillData.filter(
        (vb) => vb.balanceDue !== 0
      );

      if (location?.state?.billId) {
        updatedVendorBillData = updatedVendorBillData.filter(
          (vb) => vb._id === location?.state?.billId
        );
      }
  ...
  const handlePayData = () => {
    const totalVendorBillPay = vendorBill.reduce((acc, curr) => {
      acc = acc + +curr.billPaymentAmount;
      return acc;
    }, 0);
  ...
  const postBillPayment = () => {
    let updatedVendorBill = vendorBill.filter(
      (vb) => vb?.billPaymentAmount != 0 || vb?.billPaymentAmount != ''
    );
    const billPayments = {
      vendorId: vendor,
      paymentNo,
    ...
    if (location?.state?.edit) {
      toast
        .promise(
          httpService.put(
            `/vendortrx/updatevendorbillpayment/${location?.state?.payInfo?._id}`,
            { ...billPayments, paymentNo: location?.state?.payInfo?.paymentNo }
          ),
          {
      ...
      toast
        .promise(
          httpService.post(`/vendortrx/createvendorbillpayment`, billPayments),
          {
            ...
            success: 'Bills Payment successfull.',
                ...
                <div className="col-sm-6 col-md-6">
                  <div className="form-group">
                    ...
                    <select
                      ...
                      required
                      disabled= {location.state.vendorId}
                    >
                ...
                <div className="col-sm-6 col-md-6">
                  <div className="form-group">
                    ...
                    <input
                      ...
                      required
                      max = {Math.round(vendorBill[0]?.balanceDue)}
                    />
                  </div>
                </div>
              ...
              <div className="row">
                ...
                {modeCustom === true && (
                  <div className="col-sm-3 form-group">
                    <label>
                      Custom Payment Mode <span className="text-danger">*</span>
                    </label>
                    <input
                      name="paymentMode"
                      value={paymentMode}
