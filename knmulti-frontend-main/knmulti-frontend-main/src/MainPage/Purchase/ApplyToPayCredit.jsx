...
const ApplyToPayCredit = () => {
  ...
  let { customerPayCreditInfo, invID, invNo, invBalDue, BillData } =
    useLocation()?.state;
    ...
    if (allCredit.length !== 0) {
      let updatedCreditPay = allCredit.map((c) => {
        if (c.id === rId) {
          ...
          c.amountExcess = bal - paymentAmount;
          c.amountPaid = cred;
    ...
    setCustomerPayCreditData((prevState) => {
      return prevState.map((customerPayCreditDataItem) => {
        if (customerPayCreditDataItem._id === rId) {
          ...
          customerPayCreditDataItem.amountExcess =
            customerPayCreditDataItem.amountPaid - paymentAmount;
        }
  ...
  const handleClick = (e, record) => {
      ...
      setCustomerPayCreditData((prevState) => {
        return prevState.map((payCredit) => {
          if (payCredit._id === record._id) {
            ...
            payCredit.amountExcess += record.amc;
    ...
    if (invoiceBalance <= invBalDue) {
      if (record.amountExcess >= invoiceBalance) {
        // alert('record.excessAmount >= invoiceBalance: '+ (invBalDue - invoiceBalance));
        const amc =
          record.amountExcess === invoiceBalance
            ? record.amountExcess
            : invoiceBalance;
        ...
        setCustomerPayCreditData((prevState) => {
          return prevState.map((item) => {
            if (item._id === record._id) {
              ...
              const newExcessAmount = item.amountExcess - amc;
              updateAllCredit(
                item._id,
                item.amountExcess,
                newExcessAmount,
              ...
              item.amountExcess = newExcessAmount;
              item.amc = amc;
            }
            return { ...item };
          });
  ...
  const handleSubmit = async (e) => {
    ...
    const inv = {
      ...
      invoiceAmount: BillData?.total,
      invBalDue: invBalDue - amountToPayCredit,
      paidAmount: amountToPayCredit,
    };

    await toast
      .promise(httpService.put(``, { creditList, inv }), {
          ...
          <div className="row">
            ...
            <div className="col-auto float-right ml-auto">
              <h3 className="page-title">
                {' '}
                <span className="text-muted">Balance :-</span> ₹{invBalDue}
              </h3>
            </div>
          </div>
        </div>
