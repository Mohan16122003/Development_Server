...
const VendorCreditInfo = () => {
  ...
  const fetchVendorCreditInfo = () => {
    toast
      .promise(httpService.get(`/vendortrx/getvendorcredit?_id=${id}`), {
  ...
  const deleteVendorCredit = () => {
    toast
      .promise(httpService.delete(`/vendortrx/removevendorcredit/${id}`), {
      ...
      <>
        <div className="d-flex">
          ...
          <div class="p-2 ml-2 flex-fill">
            <div>
              ...
              <div className="ml-3">
                <Link
                  to={`/app/profile/vendor-profile/${creditData?.vendorId?._id}`}
                >
                  <div>{creditData?.vendorId?.name}</div>
                </Link>
                <div>
                  {creditData?.vendorId?.billAddress}
                </div>
              </div>
            </div>
            <hr />
        ...
        <div>
          <table class="table table-striped">
            ...
            <tbody>
              {creditData?.transaction?.length > 0 &&
                creditData?.transaction.map((trx) => (
                  <tr>
        ...
        <div className="d-flex justify-content-between">
          ...
          <div className="flex-grow-1">
            ...
            {creditData?.adjustment?.adjustmentValue !== 0 && (
              <div className="d-flex justify-content-between">
                <p className="h4 text-muted">
                  {creditData?.adjustment?.adjustmentName}
                </p>
                <p className="h4 text-muted">
                  ₹{creditData?.adjustment?.adjustmentValue}
                </p>
              </div>
            )}
            <hr />
            ...
            <div className="d-flex justify-content-between">
              ...
              <p className="h4 text-muted">
                (-)₹{creditData?.total - creditData?.balance}
              </p>
            </div>
            <div className="d-flex justify-content-between">
  ...
  return (
    <div className="page-wrapper">
      ...
      <div className="content container-fluid">
          ...
          <div className="row">
            <div className="col d-flex">
              <h3 className="page-title mr-2">
                Vendor Credit# {creditData?.creditOrder}
              </h3>
              <div>
                <span
                  className={
                    creditData?.status === 'OPEN'
                      ? 'badge bg-success text-white p-2 h5 ml-2'
                      : creditData?.status == 'PARTIAL'
                      ? 'badge bg-warning text-white p-2 h5 ml-2'
              ...
              {creditData?.status != 'CLOSED' && (
                <div className="btn btn-primary float-right mr-2">
                  <Link
                    className="text-light"
                    to={{
                      pathname: `/app/purchase/creditvbill`,
                      state: {
                        creditData: creditData,
                        vendorId: creditData?.vendorId?._id,
                      },
                    }}
                  >
                    Apply to Bills
        ...
        {viewPDF ? pdfView() : CreditView()}

        <DeleteModel title="Vendor Credit" fn={deleteVendorCredit} />
        <FileUploadModel
          modLink={`/vendortrx/updatevendorcredit/${creditData?._id}`}
          filesInfo={creditData?.fileInfos}
        />
      </div>
    </div>
  );
