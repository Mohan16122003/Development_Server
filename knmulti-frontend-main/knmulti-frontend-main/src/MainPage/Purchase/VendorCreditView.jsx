...
const VendorCreditView = () => {
  ...
  const fetchCreditOfBill = () => {
    toast
      .promise(
        httpService.get(
          `/vendortrx/getcreditofbill?vendorId=${state?.vendorId}&vendorBill=${state?.billId}`
        ),
        {
  ...
  useEffect(() => {
    if (state?.vendorId) {
      ...
      toast
        .promise(httpService.get(`/vendortrx/getvendorcredit${search}`), {
  ...
  function searchTable(data) {
    let newData = data
      .filter(
        (row) => row.creditOrder.toLowerCase().indexOf(qvc.toLowerCase()) > -1
      )
      .filter(
        (c) =>
    ...
    if (qdateto) {
      newData = newData.filter(
        (fd) => fd?.vendorCreditDate.split('T')[0] <= qdateto
      );
    }
    if (qstatus) {
      newData = newData.filter(
  ...
  const columns = [
    {
      ...
      align: 'center',
      render: (vendorCreditDate) => (
        <span>{dateFormatter(vendorCreditDate?.split('T')[0])}</span>
      ),
    },
    {
      title: 'CREDIT NOTE#',
      ...
      render: (text, record) => (
        <span>
          <Link to={`/app/purchase/vendorcreditinfo/${record._id}`}>
            {text}
          </Link>
        </span>
      ),
