...
const RecurringExpenseInfo = () => {
  ...
  const fetchRecurringExpenseInfo = () => {
    toast
      .promise(httpService.get(`/vendortrx/getrecurringexpense?_id=${id}`), {
  ...
  const deleteRecurringExpense = () => {
    toast
      .promise(httpService.delete(`/vendortrx/removerecurringexpense/${id}`), {
  ...
  const stopExpenseRecur = () => {
    toast
      .promise(
        httpService.put(
          `/vendortrx/updaterecurringexpense/${recurringExpenseData?._id}`,
          { status: 'INACTIVE' }
        ),
        {
      ...
      .then((res) => {
        setRecurringExpenseData(res?.data);
        history.goBack();
      });
  };
      ...
      <div className="content container-fluid">
          ...
          <div className="row">
            <div className="col">
              <h3 className="page-title">
                {recurringExpenseData?.profileName}{' '}
                <span className="badge bg-warning p-2 h5">
                  {recurringExpenseData?.status}
                </span>
              </h3>
            </div>
            <div className="col">
              {recurringExpenseData?.status == 'ACTIVE' && (
                <div className="rounded-circle bg-primary p-2 float-right mr-2">
                  <Link
                    to={{
                      pathname: '/app/purchase/addrecurringexpense',
                      state: { recurringExpenseData, edit: true },
                    }}
              ...
              <div className="btn btn-primary float-right mr-2">
                <Link
                  ...
                  to={{
                    ...
                    state: { recurringExpenseData, convertRecurring: true },
                  }}
                >
                  Create Expense
                </Link>
        ...
        <div className="row d-flex justify-content-evenly">
          <div className="d-flex align-items-center">
            ...
            <div className="mx-3">
              <span className="h3 text-primary">
                ₹ {recurringExpenseData?.expenseAmount}
              </span>
          ...
          <div className="ml-5 d-flex align-items-center">
            ...
            <div className="mx-3">
              <span className="h3 text-primary">
                In {recurringExpenseData?.repeatEvery?.repeatNumber}{' '}
                {recurringExpenseData?.repeatEvery?.repeatUnit} Intervals
              </span>
          ...
          <div className="ml-5 d-flex align-items-center">
            ...
            <div className="mx-3">
              <span className="h3 text-primary">
                {recurringExpenseData?.expenseNextDate?.split('T')[0]}
              </span>
          ...
          <div>
            <p className="h5 text-muted mt-1">
              ...
              <span className="h5 text-dark">
                {recurringExpenseData?.expenseStartDate?.split('T')[0]}
              </span>
            </p>
            <p className="h5 text-muted">
              Ends On{' '}
              {recurringExpenseData?.expenseEndDate ? (
                <span className="h5 text-dark">
                  {recurringExpenseData?.expenseEndDate?.split('T')[0]}
                </span>
              ) : (
        ...
        <div className="row">
          <div className="my-3">
            ...
            <div className="my-3">
              ...
              <Link
                to={`/app/profile/customer-profile/${recurringExpenseData?.customerId?._id}`}
              >
                <p className="h4">
                  {recurringExpenseData?.customerId?.displayName}
                </p>
              </Link>
            </div>
            <div className="my-3">
              ...
              <Link
                to={`/app/profile/vendor-profile/${recurringExpenseData?.vendorId?._id}`}
              >
                <p className="h4">{recurringExpenseData?.vendorId?.name}</p>
              </Link>
            </div>
            <div className="my-3">
              <p className="h4 mt-3 text-muted">
                Note : {recurringExpenseData?.notes}
              </p>
            </div>
          </div>
        </div>
