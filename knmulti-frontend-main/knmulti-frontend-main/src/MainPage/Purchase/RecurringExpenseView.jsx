...
const RecurringExpenseView = () => {
  ...
  useEffect(() => {
    toast
      .promise(httpService.get(`/vendortrx/getrecurringexpense`), {
  ...
  function searchTable(data) {
    let newData = data
      .filter(
        (c) => c?.profileName?.toLowerCase().indexOf(qname.toLowerCase()) > -1
      )
      .filter(
        (c) =>
    ...
    if (qdateto) {
      newData = newData.filter(
        (fd) => fd?.expenseNextDate.split('T')[0] <= qdateto
      );
    }

    if (qfreq) {
      newData = newData.filter(
        (s) => s.repeatEvery?.repeatUnit?.toLowerCase() == qfreq?.toLowerCase()
      );
    }
    if (qstatus) {
      newData = newData.filter(
        (s) => s.status?.toLowerCase() == qstatus?.toLowerCase()
      );
    }
    return newData;
  }
  ...
  const columns = [
    {
      ...
      render: (text, record) => (
        <span>
          <Link to={`/app/purchase/recurringexpenseinfo/${record._id}`}>
            {text}
          </Link>
        </span>
      ),
    ...
    },
    {
      ...
      render: (repeatEvery) => (
        <span>
          {repeatEvery?.repeatNumber} {repeatEvery?.repeatUnit}
        </span>
      ),
    },
    {
      ...
      align: 'center',
      render: (expenseEndDate) => (
        <span>
          {expenseEndDate ? expenseEndDate?.split('T')[0] : 'Goes Forever'}
        </span>
      ),
    },
    {
      ...
      align: 'center',
      render: (expenseNextDate) => (
        <span>{dateFormatter(expenseNextDate)}</span>
      ),
    },
    {
      title: 'Status',
