...
const VendorBill = () => {
  ...
  const [name, setName] = useState(id || state?.vendorId|| state?.owner?.id || '');
  ...
  useEffect(() => {
    if (state?.isOwner) {
      settransaction([
        {
          itemDetails: `${state?.data?.land_type} Land`,
          account: 'Land',
          quantity: state?.data?.land_area,
          unit: 'acers',
          rate: calcRate(state?.data?.land_cost, state?.data?.land_area),
          discount: { discountType: 'percent', discountValue: 0 },
          amount: state?.data?.land_cost,
        },
      ]);
    }
  }, [state]);
  const calcRate = (cost, area) => {
  ...
  const handleSubmit = (e) => {
    ...
    state?.isOwner && delete billFinalData.projectId;
    if (state?.edit) {
        ...
        .then((res) => {
          if (state?.pOConversion) {
            return httpService.put(
              `/vendortrx/updatepurchaseorder/${state?.pOData?._id}`,
              {
                status: 'CLOSED',
                billedStatus: 'BILLED',
                billInfo: res?.data?._id,
              }
            );
          } else {
            dispatch(
          ...
          <div className="col-sm-12">
            <form onSubmit={handleSubmit}>
                ...
                {state?.isOwner ? (
                  <div className="col-sm-6 col-md-3">
                    <div className="form-group">
                      <label>
                        Land Owner <span className="text-danger">*</span>
                      </label>
                      <input
                        type="text"
                    ...
                    <div className="form-group">
                      ...
                      >
                        ...
                        {vendors.map((vendor,id) => (
                          <option key={id} value={vendor._id}>
                            {vendor.name}
                          </option>
                        ))}
                      </select>
              ...
              <div className="row">
                ...
                <div className="col-sm-6 col-md-3">
                  <div className="form-group">
                    <label>Due Date</label> <span className="text-danger">*</span>
                    <div>
                      <input
                      required
                        className="form-control"
                    ...
                    <tr>
                      ...
                      <td>
                        {taxSystem === 'TCS' ? (
                          ...
                          >
                            <option value=""></option>
                            {tcstaxes.map((tcsItem,id) => (
                              <option key={id} value={tcsItem?._id}>
