...
const TransferStock = () => {
  ...
  function search(data) {
    return data
      .filter(
        (row) =>
          row?.stockId?.itemDetails.toLowerCase().indexOf(qItem.toLowerCase()) >
          -1
      )
      .filter(
        (r) =>
          r.projectFrom?.name
            ?.toLowerCase()
            .indexOf(qprojectFrom.toLowerCase()) > -1
      )
      .filter(
        (c) =>
          c.projectTo?.name?.toLowerCase().indexOf(qprojectTo.toLowerCase()) >
          -1
      );
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      align: 'center',
      render: (text, record) => (
        <span>{dateFormatter(text?.split('T')[0])}</span>
      ),
    },
    {
      title: 'Item Details',
    ...
    },
    {
      ...
      render: (text, record) => (
        <span>
          <Link to={`/app/projects/projects-view/${record?.projectFrom?._id}`}>
            {record?.projectFrom?.name}
          </Link>
        </span>
      ),
    },
    {
      ...
      render: (text, record) => (
        <span>
          <Link to={`/app/projects/projects-view/${record?.projectTo?._id}`}>
            {record?.projectTo?.name}
          </Link>
        </span>
      ),
    },
