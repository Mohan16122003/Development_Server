...
import { TICKET_STATES } from '../../model/shared/onboardingStates';
...
const Tickets = () => {
  ...
  const deleteTicket = async (id) => {
    await toast.promise(httpService.delete(`/ticket/${id}`), {
  ...
  function searchTable(data) {
    let newData = data
      .filter(
        (row) => row.title?.toLowerCase().indexOf(qtitle?.toLowerCase()) > -1
      )
      .filter(
        (r) =>
          r.department?.name?.toLowerCase().indexOf(qdep.toLowerCase()) > -1
      );

    if (qstatus) {
      newData = newData.filter(
        (s) => s.status?.toLowerCase()?.indexOf(qstatus?.toLowerCase()) > -1
      );
    }

    if (qpriority) {
      newData = newData.filter(
        (s) => s.priority?.toLowerCase()?.indexOf(qpriority?.toLowerCase()) > -1
      );
    }
    return newData;
  }
  ...
  const columns = [
    ...
    },
    {
      title: 'Last Reply',
      dataIndex: 'updatedAt',
      render: (text, record) => <p>{dateFormatter(text.split('T')[0])}</p>,
      sorter: (a, b) => {
...

    {
      ...
      render: (text, record) => (
        <div className="dropdown action-label text-center">
          ...
          >
            {Object.values(TICKET_STATES).map((status) => (
              <span
                ...
                onClick={() => {
                  httpService
                    ...
                    .then((res) => {
                      ...
                      dispatch(
                        createNotify({
                          notifyHead: `Ticket Status is ${status}`,
                          notifyBody: `Ticket Status is ${status}`,
                        })
                      );
    ...
    <div className="page-wrapper">
      ...
      {isLoading ? (
        ...
        <div className="content container-fluid">
          ...
          <TicketTracker ticketStats={ticketStats} data={data} />
...
const AddTicket = (props) => {
  ...
  //   `TKT-${Math.ceil(Math.random() * 100000)}`
  // );
  ...
  const handleDepartmentChange = (id) => {
    if (id) {
      ...
      const filteredData = props.employees?.filter(
        (v, i) => v.department === dep._id
      );
  ...
  const handleSubmit = async (e) => {
    ...
    newTicket.ticket = `TKT-${Math.ceil(Math.random() * 100000)}`;
    ...
    if (newTicket.title == undefined || newTicket.title == '') {
      toast.error('Please Enter Tickets Title');
      return;
    }
    if (newTicket.client == undefined || newTicket.client == '') {
      toast.error('Please select  client');
      return;
    }
    if (newTicket.priority == undefined || newTicket.priority == '') {
    ...
    //   return;
    // }
    if (newTicket.ticket == undefined || newTicket.ticket == '') {
      toast.error('Please Enter Tickets Number');
      return;
    }
    if (newTicket.description == undefined || newTicket.description == '') {
    ...
    if (newTicket.department == undefined || newTicket.department == '') {
      toast.error('Please select department');
      return;
    }
    if (newTicket.followers == undefined || newTicket.followers == 'Select') {
    ...
    await toast.promise(httpService.post('/ticket', newTicket), {
            ...
            <form ref={form} onSubmit={handleSubmit}>
                ...
                <div className="col-sm-6">
                  <div className="form-group">
                    ...
                    <input
                      ...
                      name="ticket"
                      type="text"
                      value={`TKT-${Math.ceil(Math.random() * 100000)}`}
              ...
              <div className="row">
                ...
                <div className="col-sm-6">
                  <div className="form-group">
                    <label>
                      Assign Staff <span style={{ color: 'red' }}> *</span>
                    </label>
                    <select
                      required
                      className="select"
              ...
              <div className="row">
                <div className="col-sm-6">
                  <div className="form-group">
                    <label>
                      Add Followers <span style={{ color: 'red' }}> *</span>
                    </label>
...
const TicketTracker = ({ ticketStats, data }) => {
  return (
    <div className="row">
      <div className="col-md-4">
        ...
        >
          ...
          <h4>
            {' '}
            {
              data?.filter((c) => c?.status === TICKET_STATES.active).length
            }{' '}
          </h4>
        </div>
      </div>
      <div className="col-md-4">
        ...
        >
          <h6>Approved Tickets</h6>
          <h4>
            {data?.filter((c) => c?.status === TICKET_STATES.approved).length}{' '}
          </h4>
        </div>
      </div>
      <div className="col-md-4">
        ...
        >
          ...
          <h4>
            {' '}
            {
              data?.filter((c) => c?.status === TICKET_STATES.closed).length
            }{' '}
          </h4>
        </div>
      </div>
                ...
                <div className="col-sm-6">
                  <div className="form-group">
                    ...
                    <input
                      ...
                      name="resolutionDeadline"
                      value={
                        selectedTicketData?.resolutionDeadline?.split('T')[0]
                      }
                      type="date"
                    />
                  </div>
