...
import { Table as TableStripped } from 'react-bootstrap';
...
const ProjectView = () => {
  ...
  const fetchLeads = async () => {
    const leadsRes = await httpService.get('/lead');
    // console.log(leadsRes.data, 'leadsRes');
    const leads = leadsRes?.data;
    // Filter Leads
    const filteredLeads = leads.filter((lead) => {
      let found = false;
      lead?.project?.map((project) => {
        if (project._id == id) {
          found = true;
        }
        return project;
      });
      return found /*&& lead?.status?.name !== 'Lead Won' && lead?.status?.name !== 'Lead Lost'*/;
    });
    // console.log(filteredLeads, ' :filteredLeads');
    setProjectLeads(filteredLeads);
  };
  ...
  const fetchProjectDetails = async () => {
    ...
    const totalAreaLeadsWons = res?.data?.subPlots
      .filter((p) => !p.sold && p.leadsInfo.some((l) => l.leadType === 'Won'))
      .reduce((a, b) => a + b.area, 0);
    ...
    <div className="page-wrapper">
        ...
        <div className="content container-fluid">
                    ...
                    <Table>
                      <TableBody>
                        ...
                        >
                          ...
                          >
                            TOTAL {projectDetails?.subtype?.toUpperCase()}{' '}
                            <br />
                            (UNITS)
                          </TableCell>
                          <TableCell
            ...
            <div className="row user-tabs">
              <div className="col-lg-12 col-md-12 col-sm-12 line-tabs">
                <ul className="nav nav-tabs nav-tabs-bottom">
                  ...
                  <li className="nav-item">
                    <a href="#docs" data-toggle="tab" className="nav-link">
                      Documents
                    </a>
                  </li>
                  <li className="nav-item">
          ...
          <div className="tab-content">
            ...
            <div className="tab-pane fade" id="docs">
              <div className="row">
                ...
                >
                  <div className="card-body">
                    <ul className="files-list">
                      <TableStripped striped bordered hover>
                        <thead>
                          <tr>
                            <th>Land Owner</th>
                            <th> Email </th>
                        ...
                        <tbody>
                          {projectDetails?.lands?.map((el, id) => (
                            <tr key={id}>
                              <a
                                href={`/app/land-bank/profile/${el.owner?._id}`}
                              >
                                {' '}
                                {el?.owner?.name}
                              </a>
                              <td>{el?.owner?.email}</td>
                              <td>{el?.owner?.phone || el.owner?.mobile}</td>
                              <td>{el.plot_no}</td>
                              <td>
                      ...
                      plot.component ? (
                        <>
                          ...
                          {plot.leadsInfo.length > 0 &&
                            ...
                            !plot.sold && (
                              <InProspectTooltip key={index} title={plot.name}>
                                <div
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    e.preventDefault();
                          ...
                          {plot.leadsInfo.length > 0 &&
                            ...
                            !plot.sold && (
                              <InSiteVisitTooltip key={index} title={plot.name}>
                                <div
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    e.preventDefault();
                          ...
                          {plot.leadsInfo.some((l) => l.leadType == 'Won') &&
                            plot.sold && (
                ...
                <Table aria-label="simple table">
                  ...
                  <TableBody>
                    {projectLeadsWon.reverse().map((l, index) => (
                      <TableRow key={index}>
                        <TableCell>
                          {l?.customer?.displayName || l?.lead?.name}
                        </TableCell>
                        <TableCell>
                          {l?.customer?.email || l?.lead?.email}
                        </TableCell>
                        <TableCell>
                          {l?.customer?.phone || l?.lead?.phone}
                        </TableCell>
            ...
            <div id="leads" className="tab-pane fade">
              <h2 className="card-title mb-0 h-100 mt-2">Leads</h2>
              <hr />
              <TableContainer component={Paper}>
                <Table aria-label="simple table">
                  <TableHead>
                  ...
                  <TableBody>
                    {projectLeads.map((l, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          {l?.currentAssigned
                            ? `${l?.currentAssigned?.firstName} ${l?.currentAssigned?.lastName}`
                            : 'Not Assigned'}
                        </TableCell>
                      ...
                      .map((l, index) => (
                        <TableRow key={index}>
                          ...
                          <TableCell className="text-center">
                            <a href="#">
                              <Link
                                to={`/app/profile/vendor-profile/${l?.vendorId?._id}`}
                              >
                                {l?.vendorId?.name}
                              </Link>
                            </a>
                  ...
                  <TableBody>
                    {tStk.reverse().map((l, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell className="text-center">
                          <Link
                            to={`/app/projects/projects-view/${l?.projectTo?._id}`}
          ...
          {plotInfoBackdrop && (
            ...
            >
              ...
              <div className="d-flex justify-content-center">
                ...
                >
                  Dimension : {plotInfo?.dimension?.height} x{' '}
                  {plotInfo?.dimension?.width}
                </h4>
                <h4
                  style={{
                    textAlign: 'center',
                        ...
                        <TableBody>
                          ...
                          {plotInfo?.leadsInfo?.map((r) => (
                            <TableRow key={r?.lead?._id}>
                              ...
                              <TableCell>
                                {r?.customer?.email || r?.lead?.email}
                              </TableCell>
                              <TableCell>
                                {r?.customer?.phone || r?.lead?.phone}
                              </TableCell>
      ...
      {allStock?.length && (
        <TransferStk
          allStock={allStock}
          pId={id}
          fetchTransferStock={fetchTransferStock}
        />
      )}
      {
        <AddPlotTable
          ...
          areaCost={projectDetails?.costPerSqFeet}
          subPlots={projectDetails?.subPlots}
        />
      }
    </div>
