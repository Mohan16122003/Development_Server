      ...
      <TableRow sx={{}}>
        <TableCell scope="row">
          <Chip
            ...
            onClick={() => {
              setSelectedPlot(row);
              setPlotInfoModal(true);
            }}
            label={row?.name}
          />
        </TableCell>
        <TableCell align="right">
          {row?.dimension?.height} x {row?.dimension?.width}
        </TableCell>
          ...
          {row?.soldTo ? (
            <Link to={`/app/profile/customer-profile/${row?.soldTo?._id}`}>
              <Chip
                sx={{ cursor: 'pointer' }}
                color="primary"
                label={`${row?.soldTo?.displayName}`}
              />
            </Link>
          ) : (
            <Chip color="primary" label={'No Customer'} />
          )}
        </TableCell>
        <TableCell align="right">
          {row?.sold ? (
    ...
    <>
      ...
      >
        {selectedPlot && ( 
          ...
          >
            <h4
              style={{ textAlign: 'center', fontWeight: 700, margin: '1rem' }}
            >
              {selectedPlot?.name}
            </h4>
            <div className="d-flex justify-content-center">
              <h4
                style={{ textAlign: 'center', fontWeight: 500, margin: '1rem' }}
              >
                Dimension : 
                {selectedPlot?.dimension?.height|| selectedPlot?.dimension} x {selectedPlot?.dimension?.width||''}
              </h4>
              <h4
                style={{ textAlign: 'center', fontWeight: 500, margin: '1rem' }}
              >
