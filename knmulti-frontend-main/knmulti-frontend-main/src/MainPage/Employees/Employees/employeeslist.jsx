...
import AddEmployeeModal from '../../Pages/Profile/modals/AddEmployeeModal';
...
const Employeeslist = (props) => {
  ...
  const fetchRolesAndDepartments = async () => {
    ...
    const workLocsRes = await httpService.get('/location');
    setWorkLocs(workLocsRes?.data);
    setRoles(roles?.data);
  ...
  const handleNewSubmit = async () => {
    ...
    if (
      employeeToAdd?.mobileNo.length >= 11 ||
      employeeToAdd?.mobileNo.length <= 9
    ) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        toast
          .promise(httpService.post('/employee', data), {
          ...
          .then((res) => {
            ...
            emailjs
              ...
              .catch((err) => {
                toast.error('Errorr from Emailjs', err.text);
              });
  ...
  const columns = [
        ...
        <h2 className="table-avatar">
          <Link to={`/app/profile/employee-profile/${record._id}`}>
            <Avatar sx={{ bgcolor: red[400] }}>
              {record?.fileInfoPic[0]?.filePath ? (
                <img
                  src={record?.fileInfoPic[0]?.filePath}
                  alt=""
                  style={{
                    height: '100%',
                    width: '100%',
    ...
    },
    {
      ...
      // dataIndex: '_id',
      align: 'center',
      render: (text, record) => <span> KN-{record._id} </span>,
