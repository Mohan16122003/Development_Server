...
const AdminDashboard = () => {
  ...
  const disAreaPer =
    ((plotAreaUnderDiscussion / totalPlotArea) * 100).toFixed(2) || 0;
  const negAreaPer =
    ((plotAreaInNegotiation / totalPlotArea) * 100).toFixed(2) || 0;
  const PlotLeadAreaPer =
    ((plotAreaLeadsWon / totalPlotArea) * 100).toFixed(2) || 0;
  const soldAreaPer = ((plotAreaSoldOut / totalPlotArea) * 100).toFixed(2) || 0;
  const inactive =
  ...
  const leftArea =
    ...
    (plotAreaUnderDiscussion +
      ...
      (plotAreaLeadsWon + plotAreaSoldOut));
  console.log({
    plotAreaInNegotiation,
    plotAreaLeadsWon,
    plotAreaUnderDiscussion,
  ...
  const fetchData = async () => {
    try {
      dashboard.project.forEach((project) => {
        ...
        console.log({ totalArea, totalAreaSold });
        const totalAreaInDiscussion = project?.subPlots
          ?.filter(
            (p) =>
              ...
              p?.leadsInfo.some(
                (l) =>
                  l?.leadType === 'Discussions' || l?.leadType === 'New Lead'
              )
          )
          .reduce((a, b) => a + b?.area, 0);
        const totalAreaInNegotiation = project?.subPlots
          ?.filter(
            (p) =>
              !p?.sold &&
              p?.leadsInfo.some((l) => l?.leadType === 'Negotiations')
          )
          .reduce((a, b) => a + b?.area, 0);
        const totalAreaLeadsWons = project.subPlots
          ?.filter(
            (p) =>
              !p?.sold && p?.leadsInfo.some((l) => l?.leadType === 'Lead Won')
          )
          .reduce((a, b) => a + b.area, 0);
        ...
        setTotalPlotArea((d) => d + totalArea || 0);
        setPlotAreaSoldOut((d) => d + totalAreaSold || 0);
        setPlotAreaInNegotiation((d) => d + totalAreaInNegotiation || 0);
        setPlotAreaLeadsWon((d) => d + totalAreaLeadsWons || 0);
        setPlotAreaUnderDiscussion((d) => d + totalAreaInDiscussion || 0);
        setTotalPlots((d) => d + totalPlots || 0);
        setTotalPlotsSoldOut((d) => d + totalPlotsSoldOut || 0);
        setTotalPlotsInDiscussion((d) => d + totalPlotsInDiscussion || 0);
        setTotalPlotsInNegotiation((d) => d + totalPlotsInNegotiation || 0);
        setTotalPlotsLeadsWon((d) => d + totalPlotsLeadsWons || 0);
      });
    } catch (error) {
      console.log(error);
    }
          ...
          <div className="row">
                ...
                <div className="card-body">
                  <div className="table-responsive">
                    <table className="table table-nowrap custom-table mb-0">
                      ...
                      <tbody>
                        {dashboard.invoices.map((inv, id) => {
                          return (
                            <tr key={id}>
                              <td>
                                <Link
                                  to={`/app/sales/invoice-view/${inv?._id}`}
                                >
                                  {inv?.invoice}
                                </Link>
                              </td>
            ...
            <div className="col-md-4 d-flex">
              <div className="card card-table flex-fill">
                ...
                <div className="card-body">
                  <div className="table-responsive">
                    <table className="table custom-table table-nowrap mb-0">
                      ...
                      <tbody>
                        {dashboard.payments.map((payment, id) => {
                          return (
                            <tr key={id}>
                              <td>
                                <Link
                                  to={`/app/sales/payment-view/${payment?._id}`}
                                >
                ...
                <div className="card-footer">
                  <Link to="/app/sales/payment-received">
                    View all payments
                  </Link>
                </div>
              </div>
