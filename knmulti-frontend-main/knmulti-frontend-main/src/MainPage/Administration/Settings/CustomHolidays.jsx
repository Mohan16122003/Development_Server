...
const CustomHolidays = () => {
  ...
  const [editholidayvalue, setEditholidayvalue] = useState(new Date())
  useEffect(async () => {
  ...
  const handleAddCustomHoliday = async(e) => {
    e.preventDefault();
    if (addCustomHoliday.title === '' || !addCustomHoliday.date);
    const res = await postCustomHoliday(addCustomHoliday);
    setData([...data, { ...res, id: data.length + 1 }]);
  };

  const handleEditCustomHoliday = async (e,id) => {
    e.preventDefault();
    ...
    let newUpdate = data.filter((el)=>el._id!==editCustomHoliday._id);
    const newData = [...newUpdate,{...res, id}].sort((a,b)=>a.id-b.id)
    setData(newData);
  };
  const handleDeleteCustomHoliday = async (e) => {
    e.preventDefault();
    if (!deleteCustomHoliday) return null;
    let res =  await removeCustomHoliday(deleteCustomHoliday?._id);
    if(!res.error){
      let newUpdate = data.filter((el)=>el._id!==editCustomHoliday._id);
      setData(newUpdate) 
    }
    ...
    <div className="page-wrapper">
      ...
      >
        <div className="modal-dialog modal-dialog-centered" role="document">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title"> Custom Holiday</h5>
              <button
                type="button"
                className="close"
                data-dismiss="modal"
            ...
            <div className="modal-body">
              ...
              <form>
                ...
                <div className="form-group">
                  ...
                  <DatePicker
                    className="form-control"
                    defaultValue= {editCustomHoliday?.date}
                    format={'dd/MM/yyyy'}
                    value={editholidayvalue}
                    onChange={(e) =>{
                      setEditholidayvalue(e)
                      setEditCustomHoliday({
                          ...editCustomHoliday,
                          date: e,
                        })
                ...
                <div className="submit-section">
                  <button
                    className="btn btn-primary submit-btn"
                    onClick={(e)=>handleEditCustomHoliday(e,editCustomHoliday.id)}
                    data-dismiss="modal"
                  >
                    Submit
                  </button>
