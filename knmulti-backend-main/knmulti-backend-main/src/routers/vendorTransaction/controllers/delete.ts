...
export const vendorBillPaymentdelete = async(req: Request, res: Response) => {
  try {
    ...
    let isProjectExists = await Land.findById(vendorBillPayment.landId).select('project')
    if(isProjectExists?.project){
      return res.status(400).json({message:'Vendor Payment Cannot be deleted as Project has been created'})
    }
    // DELETE FILE TO CLOUD
    ...
    await updateBillOnDelete(vendorBillPayment);
...
const updateBillOnDelete = async(vendor:any)=>{
  vendor?.vendorBill.forEach(async (vb:any) => {
    ...
    if(vb.billPaymentAmount != 0){
      ...
      if(paymade==0){
        await VendorBill.findByIdAndUpdate(vb._id, { balanceDue: bal, payments: paymade, status: "OPEN" }, { new: true });
      } else{
