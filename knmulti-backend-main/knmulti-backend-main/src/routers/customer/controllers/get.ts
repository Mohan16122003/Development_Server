...
import { findSubplots } from "../../lead/controllers/get";
...
export default async function controllerGet(req: Request, res: Response) {
  ...
  if (id) {
    const customer:any = await Customer.findById(id)
      ...
      .populate({path:'project', select: "_id name leadcustomers subPlots type saleStatus "})
      .populate("timeline").lean()
    if (!customer) {
      return res.status(404).send({ message: "Customer not found" });
    }
    customer.projects = findSubplots(customer.plots, customer.project)
