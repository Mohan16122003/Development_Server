...
import {
  generateSaleEstimatePDF,
  generateSaleInvoicePDF,
} from "../../../utils/pdf-generation/generatePDF";
...
export async function controllerStatusPut(req: RequestWithUser, res: Response) {
  const { id } = req.params;
  if (id) {
    try {
      ...
      if (status === "Won") {
        if (leadcustomer) {
          ...
          project.subPlots[
            project.subPlots.findIndex((p: any) => p._id === plot._id)
          ] = subPlot;
          ...
          if (leadData?.customer) {
            customer = await Customer.findOne({ lead });
          } else {
            const latest: any = await Customer.find({})
              .sort({ id: -1 })
              .limit(1);
            const cust: any = {
          ...
          leadprojects.forEach(async (leadProject: any) => {
            ...
            await Project.findByIdAndUpdate(leadProject._id, {
              subPlots: projectSubPlots,
            });
          });
      ...
      return res
        .status(400)
        .json({ msg: "Some error occured while updating the status" });
    }
  } else {
...
const createEstimate: any = async (
  project: any,
  plot: any,
  lead: any,
  customer: any,
  ...
  try {
    ...
    const uploadedEstimate = await SaleEstimate.findOne({
      _id: estimate._id,
    }).populate(["customer", "tax"]);
